...
public class Cash {
    ...
    public void addAnotherCash(Cash anotherCash){
        Map<Banknote, Integer> banknotesAnotherCash = anotherCash.getBanknotes();

        for(Banknote banknote : banknotesAnotherCash.keySet()){
            int numberOfBanknotes = banknotesAnotherCash.get(banknote);
            int prevNumberOfBanknotes = 0;

            if (getBanknotes().containsKey(banknote)) {
                prevNumberOfBanknotes = this.getBanknotes().get(banknote);
            }

            this.getBanknotes().put(banknote, numberOfBanknotes + prevNumberOfBanknotes);
            this.setCost(this.getCost() + (banknote.getCost() * numberOfBanknotes));
        }
    }
